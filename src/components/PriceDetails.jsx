...
const PriceDetails = ({ product }) => {
  ...
  return (
    ...
    <div className='flex flex-col justify-between items-center pb-2 md:flex-row'>
      <div className='flex flex-col justify-between items-center pb-2 md:flex-row'>
        <p className='font-black text-xl md:pl-4 md:text-4xl'>{product.price}</p>
        <p className='hidden md:block md:text-center md:font-bold md:text-yellow-600'>{product.stock}</p>
      </div>
      <div className='flex items-center justify-center gap-4'>
      <p className='md:hidden text-center text-sm font-bold text-yellow-600'>{product.stock}</p>
      <button onClick={() => buyProduct(product)}  className="bg-sky-800 hover:bg-yellow-600 text-white text-base font-semibold w-32 h-8 py-2 rounded-lg flex items-center justify-center md:px-4 md:text-lg md:w-full md:h-12">Add to cart</button>
      </div>
